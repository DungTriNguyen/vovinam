...
async function loadData(data) {
  ...
  for (let i of data) {
    ...
    let tenQuyen = i.tenQuyen;
    let String = `
      <tr>
          <td>${stt}</td>
          <td>${tenQuyen}</td>
          <td><a href="./tongquanphanquyenQLNND.php?maQuyen=${maQuyen}" class="phanquyen">Phân quyền</a></td>
          <td>
             <a href="#" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editNND-${maQuyen}"><i class="fa fa-edit"></i>Sửa</a>
          </td>
          <td><a href="#" class="delete-button" data-bs-toggle="modal" data-bs-target="#deleteAccountGroup-${i.maQuyen}"><i class="fa fa-trash"></i></a></td>
      </tr>
  
        `;

    let String1 = `
      <div class="modal fade" id="deleteAccountGroup-${i.maQuyen}" tabindex="-1" aria-labelledby="deleteModalLabel"
              aria-hidden="true">
              <div class="modal-dialog">
                  <div class="modal-content">
                      ...
                      <div class="modal-footer">
                          ...
                          <button type="button" data-bs-dismiss="modal" class="btn btn-danger btn-confirm-delete"  onclick="deleteByID('${i.maQuyen}')">Xóa</button>
                      </div>
                  </div>
              </div>
          </div>
        `;
    let String2 = `
        <div class="modal fade" id="editNND-${maQuyen}" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
               <div class="modal-dialog">
               <div class="modal-content">
                      <div class="modal-header">
                      ...
                      <form id="editForm">
                             ...
                             <input type="text" class="form-control" id="${maQuyen}" name="nndCode" value="${maQuyen}" disabled>
                             </div>
                             <div class="mb-3">
                             <label for="nndName" class="form-label">Tên Nhóm Người Dùng</label>
                             <input type="text" class="form-control" id="${maQuyen}-${tenQuyen}" name="nndName" value="${tenQuyen}">
                             </div>
                             <div style="text-align:right;">
                                    <button data-bs-dismiss="modal" class="btn btn-primary" id="btn-update" onclick="updateObj('${maQuyen}','${maQuyen}-${tenQuyen}',event)" >Cập nhật</button>
                             </div>   
                      </form>
                      </div>
               </div>
  ...
  try {
    ...
    let response = await fetch("../../../BLL/QuanLyNhomNguoiDungBLL.php", {
      ...
      body:
        ...
        "&maQuyen=" +
        encodeURIComponent(obj.maQuyen),
    });
...
async function deletePermissionDetail(maQuyen) {
  let response = await fetch("../../../BLL/QuanLyNhomNguoiDungBLL.php", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    ...
    body:
      "function=" +
      encodeURIComponent("deleteQuyenDetail_by_maQuyen") +
      "&maQuyen=" +
...
//Sửa một đối tượng
async function updateObj(maQuyen, tenQuyen, event) {
  event.preventDefault();

  let codePermissionValue = document.getElementById(maQuyen).value.trim();
  let namePermissionValue = document.getElementById(tenQuyen).value.trim();
