...
async function loadData(data) {
  ...
  for (let i of data) {
    ...
    let tenKhoaThi = i.tenKhoaThi;
    let ngayThi = i.ngayThi;
    let ngayKetThuc = i.ngayKetThuc;
    let hienThi = i.hienThi;
    let dsCapDaiThi = i.dsCapDaiThi;
    let ghiChu = i.ghiChu;

    // Chuyển đổi dsCapDaiThi thành một chuỗi để hiển thị
    ...
    let String = `
        <tr>
           ...
           <td>${dsCapDaiThi}</td>
           <td>${ghiChu}</td>
           <td><a href="#" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editUser-${i.maKhoaThi}"><i class="fa fa-edit"></i></a></td>
           <td><a href="#" class="delete-button" data-bs-toggle="modal" data-bs-target="#deleteUser-${i.maKhoaThi}"><i class="fa fa-trash"></i>Xóa</a></td>
        </tr>
              
          `;
    let String1 = `
        <div class="modal fade" id="deleteUser-${i.maKhoaThi}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteModalLabel">Xóa khóa thi</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    Bạn có chắc muốn xóa khóa thi này?
                    <br>
                    Mã khóa thi: ${i.maKhoaThi}
                    <br>
                    Tên khóa thi: ${i.tenKhoaThi}
                    <br>
                    Ngày thi: ${i.ngayThi}
                    <br>
                    Ngày kết thúc: ${i.ngayKetThuc}
                    <br>
                    Danh sách cấp đai thi: ${i.dsCapDaiThi}
                    <br>
                    Ghi chú khóa thi: ${i.ghiChu}
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                    <button type="button" data-bs-dismiss="modal" class="btn btn-danger btn-confirm-delete" onclick="deleteByID('${i.maKhoaThi}')">Xóa</button>
                </div>
            </div>
        </div>
    </div>
          ...
          `;
    let String2 = `
        <div class="modal fade" id="editUser-${i.maKhoaThi}" tabindex="-1" aria-labelledby="editModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">Sửa thông tin khóa thi</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editForm">
                        <div class="mb-3">
                            <label for="name" class="form-label">Mã khóa thi</label>
                            <input type="text" class="form-control" id="${i.maKhoaThi}" value="${i.maKhoaThi}"
                                name="maKhoaThi" placeholder="NCC001" disabled>
                        </div>
                        <div class="mb-3">
                            <label for="tenKhoaThi" class="form-label">Tên khóa thi</label>
                            <div class="input-group">
                            <input type="text" class="form-control" id="${i.maKhoaThi}-${i.tenKhoaThi}"
                                value="${i.tenKhoaThi}" name="tenKhoaThi" aria-describedby="togglePassword">  
                            </div>
                        </div>
                        <input type="hidden" class="form-control" id="${i.maKhoaThi}-${i.hienThi}"
                            value="${i.hienThi}" name="hienThi">
                        <div class="mb-3">
                            <label for="ngayThi" class="form-label">Ngày Thi</label>
                            <input type="date" class="form-control" id="${i.maKhoaThi}-${i.ngayThi}" value="${i.ngayThi}"
                                name="ngayThi">
                        </div>
                        <div class="mb-3">
                            <label for="ngayKetThuc" class="form-label">Ngày kết thúc</label>
                            <input type="date" class="form-control" id="${i.maKhoaThi}-${i.ngayKetThuc}"
                                value="${i.ngayKetThuc}" name="ngayKetThuc">
                        </div>
                        <div class="mb-3">
                            <label for="ghiChu" class="form-label">Ghi chú</label>
                            <input type="text" class="form-control" id="${i.maKhoaThi}-${i.ghiChu}"
                                value="${i.ghiChu}" name="ghiChu">
                        </div>
                        
                        <div style="text-align:right;">
                            <button type="submit" data-bs-dismiss="modal" class="btn btn-primary"
                                onclick="updateObj('${i.maKhoaThi}', '${i.maKhoaThi}-${i.tenKhoaThi}', '${i.maKhoaThi}-${i.ngayThi}','${i.maKhoaThi}-${i.ngayKetThuc}','${i.maKhoaThi}-${i.hienThi}','${i.maKhoaThi}-${i.ghiChu}',event)">Sửa
                                khóa thi</button>
                        </div>
                    </form>
                </div>
    ...
    try {
      ...
      if (data.mess === "success") {
        Swal.fire({
          position: "center",
          icon: "success",
          title: "Đã xóa khóa thi thành công",
...
async function addObj(event) {
  ...
  if (true) {
    window.location.href = "../../../GUI/view/admin/ThemKhoaThi.php";
  } else {
...
async function updateObj(
  ...
  if (true) {
    ...
    // let dsCapDaiValue = document.getElementById(dsCapDaiValue).value.trim();
