...
async function loadData(data) {
  ...
  let ketQuaObj = {};
  for (let i of data) {
    let maKetQuaThi = i.maKetQuaThi;
    let maMonSinh = i.maMonSinh;
    let maKhoaThi = i.maKhoaThi;
    let capDaiHienTai = i.capDaiHienTai;
    let capDaiDuThi = i.capDaiDuThi;
    let trangThaiHoSo = i.trangThaiHoSo;
    let ketQua = i.ketQua;
    let ghiChu = i.ghiChu;
    let ngayCham = i.ngayCham;
    let strketQua = "";
    let fileDuThi = i.fileDuThi ? i.fileDuThi : ""; // nếu không có file, để trống
    ...
    let strStt = "";
    if (i.trangThaiHoSo == "1") {
      strStt = `<td><a class="btn btn-sm btn-primary" title="Click để ẩn bình luận" onclick="updateStateUser('${i.maKetQuaThi}','${i.trangThaiHoSo}',event)"> Được duyệt/ Hồ sơ đạt yêu cầu<i class="fa"></i></a></td>`;
    } else {
      strStt = `<td><a class="btn btn-sm btn-danger" title="Click để hiện bình luận" onclick="updateStateUser('${i.maKetQuaThi}','${i.trangThaiHoSo}',event)"> Không được duyệt/ Hồ sơ bị loại <i class="fa"></i></a></td>`;
    }
    console.log(ketQua);
    ...
    let fileLink = fileDuThi
      ? `<a href="${fileDuThi}" download> Tải xuống </a>`
      : "Chưa có file";
    ...
    let String = `
      <tr>
         ...
         <td>${ghiChu}</td>
         <td>${ngayCham}</td>
         <td style="width: 120px;"> <input type="file" class="form-control" id="fileDuThi_${maKetQuaThi}" name="fileDuThi" accept=".doc,.docx,.xls,.xlsx,.pdf" onchange="uploadFile(event, ${maKetQuaThi})">
           ${fileLink}
         </td>
      </tr>
         
         `;
    //  <td><a href="#" class="delete-button" data-bs-toggle="modal" data-bs-target="#deleteUser-${i.maKetQuaThi}"><i class="fa fa-trash"></i>Xóa</a></td>
    let String1 = `
      <div class="modal fade" id="deleteUser-${i.maKetQuaThi}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-body">
                  ...
                  <br>
                  Tên thí sinh: ${i.maMonSinh}
                  <br>
                  Tên khóa thi: ${i.maKhoaThi}
              </div>
              <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                  <button type="button" data-bs-dismiss="modal" class="btn btn-danger btn-confirm-delete" onclick="deleteByID('${i.maKetQuaThi}')">Xóa</button>
              </div>
          </div>
      </div>
  </div>
...
async function updateStateUser(maKetQuaThi, trangThaiHoSo, event) {
  event.preventDefault();
  ...
  if (true) {
    try {
      const response = await fetch("../../../BLL/QuanLyKetQuaThiBLL.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
        body:
          ...
          "&maKetQuaThi=" +
          encodeURIComponent(maKetQuaThi) +
          "&trangThaiHoSo=" +
          encodeURIComponent(trangThaiHoSo),
      });
